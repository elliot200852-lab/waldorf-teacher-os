...
> 📌 **支援多種 AI 工具**：Claude Code 桌面版/CLI、Google Antigravity、Cursor、Windsurf 等支援 MCP 的工具皆可使用。
...
- [ ] 支援 MCP 的 AI 工具已安裝（Claude Code 桌面版、Google Antigravity、Cursor、Windsurf 等任一）
...
請根據你使用的 AI 工具，執行對應的指令（不需要手動編輯 JSON）：
...
> 兩者的 MCP 工具完全相同，差異只在設定檔位置：
> - Claude Code：`~/.claude/` 內的 MCP 設定
...
> 應該能看到你的工具出現在已設定的客戶端清單中，狀態標示為已設定。
>
> 其他支援的工具：Cursor（`nlm setup add cursor`）、Windsurf（`nlm setup add windsurf`）、Gemini CLI（`nlm setup add gemini`）。完整清單執行 `nlm setup add --help` 查看。
...
> 🖐️ **需要手動操作**：請使用者完全關閉 AI 工具（Claude Code / Antigravity / Cursor 等），然後重新開啟。MCP 伺服器會在啟動時自動載入。
