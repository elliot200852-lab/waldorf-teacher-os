...
def main():
    ...
    if has_node:
        ...
        print_success(f"Node.js 已安裝：{r.stdout.strip()}")

        if shutil.which("gws"):
            r = run_cmd(["gws", "--version"])
            gws_ver = r.stdout.strip() if r.returncode == 0 and r.stdout.strip() else "已安裝"
            print_success(f"Google Workspace CLI 已安裝：{gws_ver}")
        else:
            print_warning("Google Workspace CLI（gws）尚未安裝")
            print()
            print(_c(YELLOW, "建議安裝 gws（讓 AI 可直接操作 Google Drive、Calendar 等服務）："))
            print("  npm install -g @googleworkspace/cli")
            print()

        # 部署 OAuth client_secret
        gws_config_dir = Path.home() / ".config" / "gws"
        gws_client_secret = repo_root / "setup" / "gws-client-secret.json"

        if gws_client_secret.is_file():
            gws_config_dir.mkdir(parents=True, exist_ok=True)
            target = gws_config_dir / "client_secret.json"
            if not target.is_file():
                shutil.copy2(gws_client_secret, target)
                print_success(f"OAuth 認證檔已部署到 {gws_config_dir}/")
            else:
        ...
        if shutil.which("gws"):
            ...
            if r.returncode == 0:
                ...
                print()
                print(_c(YELLOW, "請執行以下指令登入（務必帶上你的 email）："))
                print("  gws auth login --account 你的帳號@gmail.com")
                print()
    else:
        print_info("Node.js / npm 未安裝，跳過 gws 檢查")
        print()
        print(_c(YELLOW, "若需要使用 Google Workspace CLI，請先安裝 Node.js："))
        ...
        if git_email:
            ...
            if preset_dir.is_dir():
                for preset_path in preset_dir.glob("*/env-preset.env"):
                    ...
                    if preset_env.get("USER_EMAIL") == git_email:
