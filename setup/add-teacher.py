...
class AddTeacher:
    ...
    def create_workspace(self):
        ...
        if src_personal.exists():
            ...
            # 替換佔位符：teacher_identity 基本辨識欄位
            content = dst_personal.read_text(encoding="utf-8")
            content = content.replace("  name: 填空", f"  name: {self.name}", 1)
            content = content.replace("  email: 填空", f"  email: {self.email}", 1)
            dst_personal.write_text(content, encoding="utf-8")
            print_success(f"teacheros-personal.yaml 已複製，name/email 已預填")
        else:
