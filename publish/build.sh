...
if [ "$USE_GWS_UPLOAD" = true ]; then
  ...
  for PART in "${FOLDER_PARTS[@]}"; do
    ...
    if [ -n "$FOLDER_ID" ]; then
      ...
      echo "錯誤：無法在 Google Drive 建立資料夾 ${PART}"
      rm -f "$TEMP_DOCX"
      exit 1
    fi
  done
  ...
  if [ -n "$FILE_ID" ]; then
    echo "完成。已透過 gws CLI 上傳至 Google Drive。"
    echo "雲端路徑：${GWS_FOLDER_PATH}/${CN_FILENAME}"
    echo "檔案連結：https://drive.google.com/file/d/${FILE_ID}"
  else
    echo "警告：上傳可能未成功，請至 Google Drive 確認。"
  fi
else
  # ── Google Drive Desktop 本機同步模式（原有流程）──
