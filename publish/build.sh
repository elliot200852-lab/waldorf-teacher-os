...
# Google Drive 路徑依平台分流
case "$(uname -s)" in
  Darwin)
    GDRIVE_ROOT="$HOME/Library/CloudStorage/GoogleDrive-${GOOGLE_DRIVE_EMAIL}/我的雲端硬碟/${GOOGLE_DRIVE_FOLDER}"
    ;;
  MINGW*|MSYS*|CYGWIN*|Windows_NT)
    # Windows: Google Drive Desktop 預設掛載路徑
    GDRIVE_ROOT="/g/我的雲端硬碟/${GOOGLE_DRIVE_FOLDER}"
    ;;
  *)
    echo "警告：未知平台 $(uname -s)，使用 macOS 路徑格式"
    GDRIVE_ROOT="$HOME/Library/CloudStorage/GoogleDrive-${GOOGLE_DRIVE_EMAIL}/我的雲端硬碟/${GOOGLE_DRIVE_FOLDER}"
    ;;
esac
...
TEMP_DOCX="${TMPDIR:-/tmp}/teacheros-$(date +%s)-${CN_FILENAME}"
