#!/usr/bin/env node
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// TeacherOS — 五年級古文明神話 組裝腳本
// 路徑：publish/scripts/assemble-ancient-myths.js
// 用途：將古文明神話子資料夾的 .md 組裝成完整 HTML + PDF
// 依賴：assemble-base.js（共用引擎）
// 跨平台：macOS + Windows + Linux
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//
// 用法：
//   node publish/scripts/assemble-ancient-myths.js <lesson-dir> [options]
//
// 選項：
...
//   --drive-folder=ID  Drive 目標資料夾 ID（預設：五年級古文明神話）
//   --version=v2       版本標記（跳過 Drive 清除，檔名加後綴）
//
// 組裝清單：
//   [1] content.md           — 課程正文 + 事實出處表
//   [2] waldorf-teaching.md  — 華德福教學指引
...
const SCRIPT_PATH = 'publish/scripts/assemble-ancient-myths.js';
const DRIVE_FOLDER_ID = '1lUTzAcmJ5UuI3njADbJ-a3HSLl3aUhpC';
// REQUIRED_FILES 需搭配 storyId 前綴，由 getRequiredFiles() 產生
const BASE_REQUIRED = ['content.md', 'waldorf-teaching.md', 'images.md', 'chalkboard-prompt.md', 'references.md'];
...
function parseWaldorfTeaching(waldorfTeachingMd) {
  const result = { teachingInsight: '', taiwanFunFact: '', scienceDialogue: '' };
  const sections = {};
  const sectionRegex = /^## (.+)$/gm;
  let match;
  const sectionStarts = [];
  while ((match = sectionRegex.exec(waldorfTeachingMd)) !== null) {
    sectionStarts.push({ title: match[1].trim(), index: match.index + match[0].length });
  }
  for (let i = 0; i < sectionStarts.length; i++) {
    const start = sectionStarts[i].index;
    const end = i + 1 < sectionStarts.length ? sectionStarts[i + 1].index - sectionStarts[i + 1].title.length - 3 : waldorfTeachingMd.length;
    sections[sectionStarts[i].title] = waldorfTeachingMd.slice(start, end).trim();
  }

  for (const [title, content] of Object.entries(sections)) {
    if (/教學|華德福|waldorf|指引|巧妙/i.test(title)) { result.teachingInsight = content; break; }
  }
  if (!result.teachingInsight && sectionStarts.length > 0) {
    result.teachingInsight = Object.values(sections)[0] || '';
  }
  if (!result.teachingInsight) {
    result.teachingInsight = base.stripFrontmatter(waldorfTeachingMd).trim();
  }

  for (const [title, content] of Object.entries(sections)) {
    if (/台灣|呼應|趣事|俗諺|諺語|fun/i.test(title)) { result.taiwanFunFact = content; break; }
  }
  for (const [title, content] of Object.entries(sections)) {
...
function getNextLessonTitle(storyId) {
  // TW 島嶼插曲不需要 next lesson
  if (storyId.startsWith('TW')) return '';
  const lessonNum = parseInt(storyId.replace('AM', ''), 10);
  if (isNaN(lessonNum) || lessonNum >= 25) return '';
  const nextId = 'AM' + String(lessonNum + 1).padStart(3, '0');
  const skeletonPaths = [
    path.resolve('workspaces/Working_Member/Codeowner_David/projects/ancient-myths-grade5/theme-skeleton.yaml'),
...
function assembleHtml({
  ...
  waldorfTeaching, references, images, chalkboardPrompt,
  ...
  const headContent = (headMatch ? headMatch[1] : '').replace('{{TITLE}}', `${title} — 五年級古文明神話`);
  ...
  const waldorfTeachingHtml = base.mdParagraphsToHtml(waldorfTeaching.teachingInsight);
  const taiwanFunFactHtml = waldorfTeaching.taiwanFunFact ? base.mdParagraphsToHtml(waldorfTeaching.taiwanFunFact) : '';
  const scienceDialogueHtml = waldorfTeaching.scienceDialogue ? base.mdParagraphsToHtml(waldorfTeaching.scienceDialogue) : '';
  ...
  const pullQuote = base.extractPullQuote(contentMd || '', contentSections, '每個古老文明都在追問同一個問題——我是誰？世界從何而來？');

  const html = `<!DOCTYPE html>
<html class="${season}" lang="zh-TW">
<!--
  五年級古文明神話 ${storyId} — ${title}
  TeacherOS 華德福美化文件
  季節：${season}
  生成日期：${date}
...
<header class="sticky top-0 z-40 bg-[var(--bg-wash-1)]/80 backdrop-blur-xl print:hidden">
  <div class="flex justify-between items-center w-full px-8 py-6 max-w-screen-xl mx-auto">
    <div class="flex items-center gap-3">
      ...
      <h1 class="font-headline text-3xl tracking-wide leading-relaxed text-[var(--primary)]">五年級古文明神話</h1>
    </div>
...
<main class="max-w-[900px] mx-auto px-8 py-6 relative">
  ...
  <section class="mb-6">
    <h3 class="font-headline text-xl text-[var(--primary)] mb-3 border-b border-[var(--outline-variant)]/30 pb-1.5">華德福教學指引</h3>
    <div class="space-y-3">
      ${waldorfTeachingHtml}
    </div>
    ${taiwanFunFactHtml ? `
  ...
  <section class="mb-4 text-center py-3">
    <p class="font-headline text-lg text-[var(--primary)] italic mb-3">${(() => {
      ...
      return preview || '五年級古文明神話 — 全課程完結';
    })()}</p>
    <div class="inline-flex flex-col items-center">
      <div class="w-16 h-[2px] bg-[var(--primary)]/30 mb-4"></div>
      <span class="font-label text-base text-[var(--on-surface)]">五年級古文明神話</span>
    </div>
  </section>
...
<footer class="bg-[var(--bg-wash-1)] w-full border-t-0 mt-4 pb-6">
  <div class="max-w-4xl mx-auto flex flex-col items-center gap-3 px-6 text-center">
    ...
    <div class="space-y-2">
      <p class="text-[var(--primary)] font-headline italic text-lg">五年級古文明神話 — 華德福主課程</p>
    </div>
...
// ── 主函式 ───────────────────────────────────────────
function main() {
  const opts = base.parseCLIArgs('assemble-ancient-myths.js');
  ...
  const { imagePath, imageBase64: chalkboardImageBase64, imageMime: chalkboardImageMime } =
    base.findChalkboardImage(storyId, opts.downloadsDir, chalkboardPrompt, 'AncientMyths-');
  ...
  const contentMdRaw = fs.readFileSync(path.join(resolvedDir, `${storyId}-content.md`), 'utf-8');
  const contentMd = base.stripFrontmatter(contentMdRaw);
  const waldorfTeachingMd = base.stripFrontmatter(fs.readFileSync(path.join(resolvedDir, `${storyId}-waldorf-teaching.md`), 'utf-8'));
  ...
  // extractTitle reads from raw (with frontmatter) to get YAML title:
  ...
  const waldorfTeaching = parseWaldorfTeaching(waldorfTeachingMd);
  ...
  if (!waldorfTeaching.teachingInsight) validationErrors.push('waldorf-teaching.md has no teaching insight');
  ...
  if (opts.doUpload) {
    ...
    const driveBaseName = `${storyId}${versionSuffix}-${titleSlug}`;
    const filesToUpload = [{ local: htmlPath, driveName: `${driveBaseName}.html` }];
    if (opts.doPdf) {
      const pdfPath = path.join(opts.outputDir, `${storyId}${versionSuffix}-${titleSlug}.pdf`);
  ...
  console.log(`  Waldorf:  ${waldorfTeaching.teachingInsight ? 'present' : 'missing'}`);
  ...
  if (opts.doPdf) console.log(`  PDF:      ${path.join(opts.outputDir, storyId + versionSuffix + '-' + titleSlug + '.pdf')}`);
