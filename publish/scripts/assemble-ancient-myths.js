...
//
// 參數：
//   lesson-dir        古文明神話子資料夾（如 AncientMyths_Chapters/AM001）
//
// 選項：
...
//
// 範例：
//   node publish/scripts/assemble-ancient-myths.js AncientMyths_Chapters/AM001
//   node publish/scripts/assemble-ancient-myths.js AncientMyths_Chapters/AM001 --pdf
//   node publish/scripts/assemble-ancient-myths.js AncientMyths_Chapters/AM001 --pdf --upload
//   node publish/scripts/assemble-ancient-myths.js AncientMyths_Chapters/AM001 --season=autumn --pdf
//   node publish/scripts/assemble-ancient-myths.js AncientMyths_Chapters/AM001 --dry-run
//
// 組裝清單：
...
//
// 版本：1.0.0 (2026-03-24) — fork from assemble-story.js v2.2.0，古文明神話專用
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// Google Drive 上傳設定
const DRIVE_FOLDER_ID = '1lUTzAcmJ5UuI3njADbJ-a3HSLl3aUhpC'; // Google Drive「古文明神話五年級」
...
function extractPullQuote(contentMd, contentSections) {
  ...
  // fallback
  return '每個古老文明都在追問同一個問題——我是誰？世界從何而來？';
}
...
function parseImages(imagesMd) {
  const sections = [];
  // 支援 ### N. 格式（AM001 風格）與 ## 圖 N： 格式（AI 自動生成風格）
  ...
  for (const block of blocks) {
    ...
    //   格式 C（古文明神話）：- 連結：https://url（獨立行）
    let url = '';
    // 格式 C：獨立的「連結」欄位（古文明神話格式）
...
function parseWaldorfTeaching(waldorfTeachingMd) {
  ...
  // 教學指引（匹配含「教學」或「華德福」或「waldorf」或「指引」的標題）
  for (const [title, content] of Object.entries(sections)) {
    if (/教學|華德福|waldorf|指引|巧妙/i.test(title)) {
...
${headContent}
<style>
/* ancient-myths-daily PDF 分頁修正 — 確保內容從第一頁開始流動 */
@media print {
...
function main() {
  ...
  if (!storyDir) {
    console.error('Usage: node assemble-ancient-myths.js <lesson-dir> [options]');
    console.error('');
    ...
    console.error('  --drive-folder=ID  Drive folder ID (default: 五年級古文明神話)');
  ...
  const requiredFiles = ['content.md', 'waldorf-teaching.md', 'images.md', 'chalkboard-prompt.md', 'references.md'];
  ...
  if (!chalkboardImagePath) {
    const pattern = new RegExp('AncientMyths-' + storyId + '.*chalkboard', 'i');
    try {
  ...
  const kovacsTeaching = parseWaldorfTeaching(waldorfTeachingMd);
  ...
  if (!kovacsTeaching.teachingInsight) {
    validationErrors.push('waldorf-teaching.md has no teaching insight');
  }
  ...
  console.log(`  Waldorf:  ${kovacsTeaching.teachingInsight ? 'present' : 'missing'}`);
