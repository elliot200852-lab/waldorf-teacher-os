...
當教師說出「開工」「開始」「早安」「我來了」「準備好了」「start」「let's go」「上工」等開場語時，**立即讀取 `ai-core/skills/opening.md` 並依照其中的步驟執行**。

Opening 技能會引導你完成：
1. 確認 Repo 是否為最新版本（無終端機時提醒教師手動 `git pull`）
2. 讀取 `ai-core/AI_HANDOFF.md`（完整載入序列）
3. 依序載入必讀檔案（teacheros.yaml → foundation → acl → personal → DI 框架）
4. 掃描教師個人技能（`{workspace}/skills/`）
5. 報告系統狀態與上次工作紀錄

**不要跳過這個步驟。** 即使你的 INSTRUCTIONS.md 已包含教師身份與系統狀態，Opening 技能仍可能需要更新進度錨點或執行 git pull。

### 對話中的技能觸發

對話過程中，教師的口語指令可能觸發其他技能。完整的觸發語對照表在 `ai-core/AI_HANDOFF.md` 的「技能執行規則」段落。常用觸發語：
