...
資料源清單：
  ...
  8. 臺灣主權AI訓練語料庫 (TAIC) — 數發部，正體中文主權語料（Beta）
...
class HistoryItem:
    """統一的歷史素材資料結構"""
    source: str              # 資料來源代碼 (tcmb/archives/nhrm/tm/datagov/boch/openmuseum/taic)
    title: str               # 標題
...
class TAICAdapter:
    ...
    臺灣主權AI訓練語料庫 (TAIC)
    營運：數位發展部（MODA）
    狀態：Beta（2026）。目前無正式 REST API，產生搜尋連結。
    規模：~12 億 token / 3,700+ 資料集
    涵蓋：文化藝術、語言詞彙、歷史文物、在地文化、原住民族、學術內容
    授權：開放資料軌（免註冊）+ 授權AI訓練資料軌（需註冊）
    詳見：ai-core/reference/corpus-taic.yaml
    """
    SOURCE = "taic"
    BASE = "https://taic.moda.gov.tw"

    def search(self, keyword: str) -> list[HistoryItem]:
        return [HistoryItem(
            source=self.SOURCE,
            title=f"TAIC 主權語料庫：{keyword}",
            description=(
                "數發部主權AI訓練語料庫。涵蓋歷史文物、在地文化、原住民族、"
                "文化藝術等領域，來自中研院、原民會、國美館等 50+ 機構。"
                "開放資料軌可自由使用。建議以瀏覽器搜尋或 AI WebSearch 查詢。"
            ),
            url=f"{self.BASE}",
            category="主權語料",
            license="開放資料 / 授權AI訓練（雙軌）",
        )]
...
class TaiwanHistoryAPI:
    ...
    def search_for_lesson(self, topic: str, grade: int = 9, era: str = "") -> dict:
        ...
        result["references"] = self.search(keyword, sources=["datagov", "taic"])
        return result
    ...
    def to_markdown(items: list[HistoryItem], title: str = "") -> str:
        ...
        source_names = {
            ...
            "openmuseum": "開放博物館", "taic": "主權語料庫（TAIC）",
        }
