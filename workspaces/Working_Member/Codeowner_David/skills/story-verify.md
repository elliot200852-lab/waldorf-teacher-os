...
1. `{workspace}/projects/stories-of-taiwan/stories/[區塊]/[ID]/[ID]-content.md`
2. `{workspace}/projects/stories-of-taiwan/stories/[區塊]/[ID]/[ID]-narration.md`
3. `{workspace}/projects/stories-of-taiwan/stories/[區塊]/[ID]/[ID]-images.md`
4. `{workspace}/projects/stories-of-taiwan/stories/[區塊]/[ID]/[ID]-raw-materials.md` — 對照素材包
5. `{workspace}/projects/stories-of-taiwan/index.yaml` — 前後篇脈絡

## 入口驗證（由 story-daily pipeline 強制執行）

**權威來源：** `story-daily.md` Checkpoint 矩陣 Step 3→4 列。以下為快速參照，若有衝突以 checkpoint 矩陣為準。

啟動本技能前，必須確認以下條件全部滿足，否則回報 FAIL 並停止：

1. `stories/[區塊]/[ID]/[ID]-content.md` 存在且字數 > 300
2. `stories/[區塊]/[ID]/[ID]-narration.md` 存在
3. `stories/[區塊]/[ID]/[ID]-images.md` 存在且至少包含 1 張主圖（含「來源」欄位）
4. `stories/[區塊]/[ID]/[ID]-chalkboard-prompt.md` 存在且含 `## English Prompt` 段落

## 執行步驟

### Step 1 — 事實查核（Fact Check）

逐一檢查 [ID]-content.md 中的每個史實錨點：
...
| 檢查項目 | 標準 | 結果 |
|----------|------|------|
| 故事本文字數合理（不設硬性上限，以自然篇幅為準） | 必須 | PASS/FAIL |
