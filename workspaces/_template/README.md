...
> 本資料夾是你的**個人教學工作空間**。
> David 會透過 `add-teacher.sh` 自動建立你的工作空間，
...
第 1 層：共享參考庫（Shared Reference）
  └─ ai-core/teacheros.yaml
     ai-core/reference/pedagogy-framework.yaml
     ai-core/reference/subject-*.yaml
  └─ 系統標準、最佳實踐、理論框架
...
David（管理員）會執行 `bash setup/add-teacher.sh`，自動為你建立工作空間：
...
2. **`teacheros-personal.yaml`** — 個人教學哲學
   - 教育信念、教學方法、科目理念
   - AI 系統會在每次對話時讀取，用來理解你的教學風格

3. **`philosophy.yaml`** — 簡化版（可選）
   - 如果你只想快速覆寫某些欄位，用這個
   - 包含了 teacheros-personal.yaml 的核心欄位
...
| **`teacheros-personal.yaml`** | 詳細教學哲學、課堂結構、科目理念（AI 長期參考） | 中等 |
| **`philosophy.yaml`** | 簡化版，只填核心欄位（可選） | 簡單 |
...
│   ├── teacheros-personal.yaml   ← 個人教學哲學
│   ├── philosophy.yaml           ← 簡化版
│   └── projects/
...
└── Working_Member/               ← 所有活躍教師的工作空間
    ...
    │       └── class-9d/
    │
    └── Teacher_林信宏/            ← 你的工作空間（由 add-teacher.sh 建立）
        ├── workspace.yaml
        ├── teacheros-personal.yaml
        └── projects/
            └── class-8a/
