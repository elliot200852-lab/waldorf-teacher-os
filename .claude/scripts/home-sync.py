...
SECTION_ANCHORS = [
    ...
    ("ancient-myths-grade5/",           "### 古文明神話（五年級）"),
    ("botany-grade5/",                  "### 五年級植物學（botany-grade5）"),
    ("grade3-austronesian-myths/",      "### 南島神話主課程（三年級）"),
    ("taiwanese-history/",              "### 臺灣歷史課程文稿（taiwanese-history）"),
    ("Kovacs_Books/",                   "### Kovacs 系列教材"),
    # 其他教師
    ("Teacher_郭耀新/",                  "## 郭耀新老師的工作空間"),
    ("Teacher_陳佩珊/",                  "## 陳佩珊老師的工作空間"),
    ("Teacher_劉佳芳/",                  "## 劉佳芳老師的工作空間"),
    ("Teacher_林詩閔/",                  "## 林詩閔老師的工作空間"),
    ("Teacher_林雅婷/",                  "## 林雅婷老師的工作空間"),
    ("Teacher_莊宜瑾/",                  "## 莊宜瑾老師的工作空間"),
    ("Teacher_張銘分/",                  "## 張銘分老師的工作空間"),
    ("Teacher_李佳盈/",                  "## 李佳盈老師的工作空間"),
    ("Teacher_張仁謙/",                  "## 張仁謙老師的工作空間"),
    # 系統
...
SKIP_PATTERNS = [
    ...
    "temp/",
    ".DS_Store",
    "pipeline-status",    # 暫存檔，不收錄
    "current-task",       # 暫存檔，不收錄
    "quality-log",        # 已在專案區段收錄
]
...
def generate_description(filepath):
    ...
    # 從路徑推斷更具體的描述，絕不使用「自動收錄」
    if "pipeline-status" in filepath:
        return "管線狀態追蹤"
    if "quality" in filepath or "reviews/" in filepath:
        return "品質審核紀錄"
    ...
    if "skills/" in filepath:
        return "技能定義"
    if "台語" in filepath or "台語課" in filepath:
        return "台語課教學資料"
