...
> 技術架構看 [[HOME|全站檔案總覽]]，哲學根基看 [[AI 時代華德福教師的定位與價值]]。
...
| 層次 | 問的問題 | 對應檔案 |
|------|---------|---------|
| 哲學層 | 人是什麼？教育為了什麼？ | [[ai-core/teacheros-foundation.yaml\|teacheros-foundation.yaml]] |
| 發展層 | 這個年齡的學生正在經歷什麼？ | [[ai-core/reference/pedagogy-framework.yaml\|pedagogy-framework.yaml]] |
| 科目層 | 這個學科如何成為心靈的解藥？ | [[ai-core/reference/subject-english.yaml\|subject-english.yaml]]、[[ai-core/reference/subject-history.yaml\|subject-history.yaml]] |
...
完整框架見 [[projects/_di-framework/project.yaml|DI 框架 project.yaml]]。
...
這些原則來自實際建立 Skill 的歷程，完整實戰紀錄見 [[教師AI備課指南-從零建立你的第一個Skill]]。
...
技能正本見 [[lesson-engine]]，英文科覆蓋層見 [[english-overlay]]。
...
技能目錄見 [[ai-core/skills/README|技能總目錄 README]]。
觸發語對照見 [[skills-manifest]]。

### 系統技能 vs 個人技能

| 類型 | 路徑 | 誰能改 | 範例 |
|------|------|--------|------|
| 系統技能 | `ai-core/skills/` | 只有管理員 | [[opening]]、[[lesson]]、[[session-end]] |
...
`lesson-engine` 是通用設計引擎，定義五階段工作流。各科目可建立覆蓋層（overlay），補充科目專屬的原則與規格。

你說「設計一堂九年級英文課」  
↓  
引擎載入 → lesson-engine.md（通用五階段流程）  
↓  
Step 0 自動載入覆蓋層 → english-overlay.md（英文科專屬原則）
↓  
四階段執行：研究 → 設計（暫停確認）→ 審核 → 產出  
↓  
輸出：教師教案.md + 學生學習單.md

建立新覆蓋層的範本見 [[workspaces/_template/skills/EXAMPLE-subject-overlay.md|EXAMPLE-subject-overlay]]。

### 建立 Skill 的實戰步驟

> [!tip] Skill 是「長」出來的，不是「寫」出來的
...
完整的實戰紀錄（從第一版到第三版的演變）見 [[教師AI備課指南-從零建立你的第一個Skill]]。

### 新增一個系統技能的流程

1. 在 `ai-core/skills/` 新建 `.md` 檔，包含：觸發條件、執行步驟、輸出格式
2. 在 [[skills-manifest]] 加一行索引
...
三層記憶模型的完整操作指引見 [[TeacherOS × 專案三層記憶系統指引手冊]]。
...
對應技能：[[opening]] → [[load]] → [[lesson]] / [[syllabus]] / [[homeroom]] → [[session-end]]
...
AI 入口文件 [[AI_HANDOFF]] 是所有 AI 的起點——30 秒進入工作狀態。
...
| 時機 | 做什麼 | 參考 |
|------|--------|------|
| 每次對話結束 | 更新 session.yaml 進度錨點 | [[session-end]] |
| 每個教學區塊結束 | Block-end 反思：學生反應、教學調整、DI 效果 | [[block-end]] |
| 每學期 | 架構回顧：哪些 Skill 有效、哪些需要修正 | [[architecture-review-2026-03-04]] |

### Skill 的迭代原則

1. **每跑一次就可能發現新原則** — 記下來，更新 Skill
2. **學生的反應是最終檢驗標準** — 教案好不好用，教室裡見真章
3. **品質標準要定期校準** — 見 [[strategy-output-quality-standard]]
...
|------|------|
| 全站檔案索引 | [[HOME\|全站檔案總覽]] |
| 華德福教師在 AI 時代的定位 | [[AI 時代華德福教師的定位與價值]] |
| 學生發展與課程連結 | [[華德福七至九年級學生生命樣態與課程的連結]] |
| 三層記憶系統操作指引 | [[TeacherOS × 專案三層記憶系統指引手冊]] |
| Skill 建立實戰紀錄 | [[教師AI備課指南-從零建立你的第一個Skill]] |
| 差異化教學框架 | [[projects/_di-framework/project.yaml\|DI 框架 project.yaml]] |
| 技能總目錄 | [[ai-core/skills/README\|技能總目錄 README]] |
| 新教師環境設定 | [[setup/teacher-guide.md\|teacher-guide.md]] |
