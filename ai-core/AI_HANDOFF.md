...
2. `ai-core/teacheros-foundation.yaml`
   → 華德福教育共用根基（精簡版：設計原則 + 年級發展 + DI 框架）。
   深度教育哲學工作時，按檔案底部 `_extended_version` 指引載入完整版。
   → **所有教師共讀，是課程設計的思想底層**
...
| 「傳給」「傳檔案給」「收檔案」「收 Drive」「分享 Drive 檔案」「drive transfer」 | `ai-core/skills/drive-transfer.md` |
...
| 「文學課設計」「語文課」「臺灣文學」「文學主課程」「literature」 | `ai-core/skills/literature-overlay.md`（overlay，由 lesson-engine 自動載入） |
...
| 「整合新文件」「消化 raw」「消化文件」「raw-ingest」「整理素材」「把文件放進 wiki」「消化這份文件」 | `ai-core/skills/raw-ingest.md` |
| 「備課反思」「設計反思」「wiki reflect」「寫反思」「記錄設計思考」「反思一下」 | `ai-core/skills/wiki-reflect.md` |
...
**Google Workspace 操作：** GWS CLI 的安裝與認證由教師自行在個人環境完成（不再由 TeacherOS 系統統一設定）。AI 執行 Google Workspace 操作時的指令參考見 `ai-core/reference/gws-cli-guide.md`。
...
教師的本機不一定安裝了 git hook（尤其 Windows + Antigravity 使用者），伺服器端的 GitHub Action 只能事後通報，無法阻擋已 push 的 commit。**AI 是這些教師唯一的即時防線。**
