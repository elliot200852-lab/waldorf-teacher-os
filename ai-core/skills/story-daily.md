...
- Subagent 讀取 content.md、narration.md、images.md、chalkboard-prompt.md、raw-materials.md
...
**5.9** 等待圖片生成（每 5 秒截圖檢查，上限 60 秒）。**重要：每次截圖後必須先向下捲動頁面（scroll down 3-5 ticks），才能看到生成的圖片——Gemini 生成完成後圖片在畫面下方，不往下捲會誤以為還在生成中。** 判斷標準：截圖中出現完整圖片（非載入動畫）
...
2. 模糊匹配：檔名含 storyId + "chalkboard"
3. Gemini 預設檔名：`Gemini_Generated_Image_*`，取 30 分鐘內最新
4. 任何含 storyId 的圖檔
若 4 種策略全部失敗 → 重試整個 Step 5（最多 3 次）

**完成條件：** ~/Downloads/ 中存在 `AssemblyStory-[ID]-chalkboard.png`（> 50KB），且 `chalkboard-prompt.md` 的下載檔名欄位已填入。
