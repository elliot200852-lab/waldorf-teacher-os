...
### 當第三參數為 `45`、`90` 或 `120` → 課程設計模式

1. 讀取統一課程引擎：`ai-core/skills/lesson-engine.md`
2. 引擎 Step 0 自動偵測格式（45/90/120）並載入對應節奏模板
3. 依引擎 Step 0 載入科目覆蓋層（先找 `ai-core/skills/[科目]-overlay.md`，再找 `[科目]-45.md`，再找 `{workspace}/skills/draft-[科目]-overlay.md`）
4. 若覆蓋層都不存在 → 提示教師：「目前沒有 [科目] 的覆蓋層，將僅使用通用引擎設計。」
5. 將班級代碼傳入引擎 Step 0，由引擎接管後續五階段工作流
